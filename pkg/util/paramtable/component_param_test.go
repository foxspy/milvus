...
func TestQueryNodeStrictGroupSettings(t *testing.T) {
	...
	assert.False(t, cfg.EnableSearchPath.GetAsBool())
	assert.Equal(t, 1, cfg.EnableSearchPathK.GetAsInt())
	params.Save(cfg.EnableSearchPath.Key, "true")
	params.Save(cfg.EnableSearchPathK.Key, "17")
	assert.True(t, cfg.EnableSearchPath.GetAsBool())
	assert.Equal(t, 17, cfg.EnableSearchPathK.GetAsInt())
	params.Reset(cfg.EnableSearchPath.Key)
	params.Reset(cfg.EnableSearchPathK.Key)
	assert.False(t, cfg.EnableSearchPath.GetAsBool())
	assert.Equal(t, 1, cfg.EnableSearchPathK.GetAsInt())
}
