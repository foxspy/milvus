...
MemVecIndexTranslator<T>::get_cells(
    ...
    auto index = storage_type_ == milvus::cachinglayer::StorageType::MEMORY
                     ? load_binary_set_in_memory()
                     : load_binary_set_in_mmap();
...
template <typename T>
std::unique_ptr<knowhere::Index<knowhere::IndexNode>>
MemVecIndexTranslator<T>::load_binary_set_in_memory() const {
...
template <typename T>
std::unique_ptr<knowhere::Index<knowhere::IndexNode>>
MemVecIndexTranslator<T>::load_binary_set_in_mmap() const {
    auto filepath =
