...
class SegmentExpr : public Expr {
 public:
    ...
    ProcessIndexChunksForValid() {
        using IndexInnerType = std::conditional_t<std::is_same_v<T, std::string_view>, std::string, T>;
        using Index = index::ScalarIndex<IndexInnerType>;
        int processed_rows = 0;
        TargetBitmap valid_result;
        valid_result.set();
    ...
    CanUseIndex(OpType op) const {
        ...
        if (op == OpType::Match) {
            for (size_t i = current_index_chunk_; i < num_index_chunk_; i++) {
                ...
                if (!index_ptr->SupportRegexQuery() && !index_ptr->HasRawData()) {
                    return false;
                }
 ...
 protected:
    segcore::SegmentInternalInterface* segment_;
