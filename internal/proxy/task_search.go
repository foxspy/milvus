...
func (t *searchTask) initSearchRequest(ctx context.Context) error {
	...
	t.PlaceholderGroup, err = t.convertPlaceholderIfNeeded(t.request.GetPlaceholderGroup(), t.FieldId)
	if err != nil {
		return err
	}
	...
	if embedding.HasNonBM25Functions(t.schema.Functions, []int64{queryInfo.GetQueryFieldId()}) {
...
func (t *searchTask) tryGeneratePlan(params []*commonpb.KeyValuePair, dsl string, exprTemplateValues map[string]*schemapb.TemplateValue) (*planpb.PlanNode, *planpb.QueryInfo, int64, bool, error) {
	...
	if err != nil || len(annsFieldName) == 0 {
		...
		if len(vecFields) == 0 {
			return nil, nil, 0, false, merr.WrapErrParameterInvalidMsg(AnnsFieldKey + " not found in schema")
		}
